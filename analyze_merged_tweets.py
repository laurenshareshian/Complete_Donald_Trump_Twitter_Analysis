...
#    print(j, js[j]['text'], js[j]['id'],js[j]['created_at'] )
    tweetcount=tweetcount+1
    line=remove_punctuation(js[j]['text'])
    line = line.lower()
    words = line.split()
    for word in words:
...
mintime=min(date_list)
maxtime=max(date_list)
print()
print('number of tweets analyzed =', tweetcount)
print('between the dates:', datetime.datetime.strptime(mintime.strftime("%d %b %Y"), '%d %b %Y'), datetime.datetime.strptime(maxtime.strftime("%d %b %Y"), '%d %b %Y'))
